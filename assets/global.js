class Counter {
  constructor({threshold = .5, sectionSelector = '.page__container', makeBlocks = false, repeat = false} = {}) {
    this.makeBlocks = makeBlocks;
    let observer = new IntersectionObserver((entries, observer) => {
      entries.forEach(entry => {
        if(entry.isIntersecting) {
          const targetElement = entry.target;
          const counterItems = targetElement.querySelectorAll('[data-counter-item]');
          if(counterItems.length) {
            this.init(counterItems);
          }
  ...
  init(items = null) {
    let digitItems = items || document.querySelectorAll('[data-counter-item]');

    if(digitItems) {
      digitItems.forEach(item => { 
        const matches_array = item.innerHTML.match(/([^\d]*)([\d]*)([^\d]*)/);
        this.animate(item, +(matches_array[2]), matches_array[1], matches_array[3]);
      });
    }
  };

  animate(item, value, prefix, suffix) {
    let startTime = null;
    const duration = parseInt(item.dataset.counterItem) || 1000;
    ...
    const step = (timeStamp) => {
      ...
      if(this.makeBlocks) {
        item.innerHTML = `<span>${prefix}</span>${Math.floor(progress * (startPosition + finishValue))}<span>${suffix}</span>`;
      } else {
        item.innerHTML = `${prefix}${Math.floor(progress * (startPosition + finishValue))}${suffix}`;
      }
